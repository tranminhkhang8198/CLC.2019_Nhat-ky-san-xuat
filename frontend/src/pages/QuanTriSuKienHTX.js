...
import { ListItems } from '../components/DataTable/QuanTriSuKienHTXDataTable';
import DeleteItemsModal from '../components/Modals/DeleteItemsModal';
import AddItemModal from '../components/Modals/AddItemModal';

class QuanTriSuKienHTX extends Component {
  constructor(props) {
    super(props);
    this.state = {
      error: null,
  ...
  async componentDidMount() {
    const cooperativeEvents = await this.getData();
    this.setState({
      data: cooperativeEvents.data,
      totalPages: cooperativeEvents.totalPages,
    });
  }
  ...
  async componentDidUpdate() {
    ...
    if (refresh) {
      const cooperativeEvents = await this.getData();
      this.updateDataWhenRendered(cooperativeEvents);
    }
  }
